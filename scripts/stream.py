...
from audio_engine import AudioEngine, generate_audio_files
...
def ffmpeg_cmd(display: int, width: int, height: int,
               fps: int, rtmp_url: str, audio_pipe: str = "") -> list:
    # Audio source: named pipe with cricket atmosphere (ambient + events)
    if audio_pipe and os.path.exists(audio_pipe):
        audio_input = [
    ...
    return [
        ...
        "-s", f"{width}x{height}",
        "-i", f":{display}.0+0,0",
        # Draw mouse off screen so cursor never appears
        "-draw_mouse", "0",
    ] + audio_input + [
        # Video encoding — FIX: larger bufsize=18000k prevents buffering/drops
        # ultrafast+zerolatency minimises encode latency for live streaming
        "-vcodec", "libx264",
        "-preset", "ultrafast",           # FIX: was veryfast — ultrafast reduces encoder delay
        "-tune", "zerolatency",
        "-maxrate", "4500k",
        "-bufsize", "18000k",             # FIX: was 9000k — 4x bitrate buffer stops buffering
        "-pix_fmt", "yuv420p",
        "-g", str(fps * 2), "-keyint_min", str(fps),
        "-sc_threshold", "0",
        "-x264-params", "nal-hrd=cbr",   # FIX: constant bitrate mode for stable stream
        # Audio encoding
        "-acodec", "aac", "-ab", "128k", "-ar", "44100",
        # Output — FIX: reconnect flags so FFmpeg auto-reconnects on network blip
        "-f", "flv",
...
def launch_chromium(display_num: int, html_file: Path,
    ...
    p = subprocess.Popen([
        ...
        "--blink-settings=cursorVisibility=0",  # hide cursor from stream
...
def run_stream(label: str, display_num: int, width: int, height: int,
    ...
    # ── Audio engine ──────────────────────────────────────────────────────
    pipe_path = f"/tmp/xstar_audio_{label.lower()}.pipe"
    audio = AudioEngine(data_file, pipe_path, AUDIO)
    audio.start()
    # Wait briefly for pipe to exist before FFmpeg opens it
    for _ in range(20):
        if os.path.exists(pipe_path):
            break
        time.sleep(0.1)
    log.info(f"[{label}] Audio engine ready: {pipe_path}")
    ...
    cmd    = ffmpeg_cmd(display_num, width, height, fps, rtmp_url, pipe_path)
    ...
    while not stop_event.is_set():
        ...
        if ffmpeg.poll() is not None:
            ...
            # Ensure audio pipe still exists for restarted FFmpeg
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stream-target",default="both",
                        choices=["both","main","shorts"],
                        help="Which streams to run: both/main/shorts")
    args = parser.parse_args()
    # Also check env var (set by workflow)
    import os as _os
    if args.stream_target == "both":
        args.stream_target = _os.environ.get("STREAM_TARGET", "both")

    token_json = os.environ.get("YOUTUBE_TOKEN_JSON", "")

    if not token_json:
        log.error(
            "YOUTUBE_TOKEN_JSON secret is not set.\n"
            "  Run once:  python backend/youtube_broadcast.py --auth\n"
            "  Then add the token.json contents as GitHub secret: YOUTUBE_TOKEN_JSON\n"
            "  No RTMP keys needed — API generates the stream URL automatically."
        )
        sys.exit(1)

    sys.path.insert(0, str(BACKEND))
    log.info("Mode: API-only broadcast (YOUTUBE_TOKEN_JSON)")
    ...
    if main_session.available:
        ...
        log.info("Main session ready — RTMP URL obtained from API (no manual key needed)")
    else:
        log.error("Main StreamSession failed — check YOUTUBE_TOKEN_JSON and API quota")
        sys.exit(1)

    shorts_session = StreamSession(is_shorts=True)
    if shorts_session.available:
        shorts_rtmp = shorts_session.rtmp_url
        log.info("Shorts session ready")
    else:
        log.warning("Shorts session unavailable — skipping shorts stream")

    # ── Launch threads ────────────────────────────────────────────────
    stop    = threading.Event()
    threads = []

    target = args.stream_target
    log.info(f"Stream target: {target}")
    ...
    if target in ("both", "main"):
        ...
        log.info("Skipping MAIN stream (target=shorts)")

    if target in ("both", "shorts") and shorts_rtmp:
        t2 = threading.Thread(
            target=run_stream,
        ...
        threads.append(t2)
    elif target in ("both", "shorts") and not shorts_rtmp:
        log.warning("Shorts requested but no RTMP URL — skipping")

    if len(threads) == 2:
        log.info("Dual stream: MAIN@24fps + SHORTS@20fps")
    elif len(threads) == 1:
        log.info(f"Single stream: {threads[0].name}")
