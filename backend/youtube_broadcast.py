...
class StreamSession:
    ...
    def _req(self, method, url, body=None, params=None):
        ...
        try:
            ...
            # Log full body so we can diagnose exactly what YouTube rejected
            log.error(f"YouTube API {method} {url} → HTTP {e.code}")
            log.error(f"Full error response: {err_body}")
            # Parse YouTube error if JSON
            try:
                err_json = json.loads(err_body)
                yt_err = err_json.get("error", {})
                code = yt_err.get("errors", [{}])[0].get("reason", "")
                msg  = yt_err.get("message", "")
                if "liveStreamingNotEnabled" in code or "liveStreamingNotEnabled" in err_body:
                    log.error("⛔ FIX: Live Streaming is NOT enabled on this YouTube channel.")
                    log.error("   Go to: YouTube Studio → Settings → Channel → Feature eligibility")
                    log.error("   Enable 'Live streaming' — may require phone verification.")
                elif "forbidden" in code.lower() or e.code == 403:
                    log.error("⛔ FIX: OAuth token lacks youtube scope. Re-run: python backend/youtube_broadcast.py --auth")
                elif "quotaExceeded" in code:
                    log.error("⛔ FIX: YouTube API quota exceeded. Wait 24h or request quota increase.")
                elif msg:
                    log.error(f"YouTube error reason: {code} — {msg}")
            except Exception:
                pass
            raise RuntimeError(f"HTTP Error {e.code}: {err_body[:500]}")
    ...
    def _create_stream(self):
        ...
        try:
            stream = self._req("POST", f"{API}/liveStreams", body={
                ...
                "cdn": {
                    # BOTH frameRate AND resolution are REQUIRED by YouTube API when ingestionType=rtmp
                    # Using "variable" for both works on all channel types without verification
                    "frameRate":     "variable",
                    "resolution":    "variable",
                    "ingestionType": "rtmp",
                },
