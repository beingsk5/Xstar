...
CORRECT ARCHITECTURE:
  One liveStream (= one permanent RTMP URL) persists for the entire job.
  Multiple liveBroadcasts are created over time (one per match) and each
  is bound to the same liveStream. When a match changes, old broadcast ends,
  new one is created and bound to the same stream. FFmpeg never restarts.

  StreamSession
    .rtmp_url          ← permanent, set once, given to FFmpeg
    .create_broadcast(match_data)  ← call at match start
    .go_live()         ← after FFmpeg has been streaming ~10s
    .end_broadcast()   ← at match end
    .promote(match_data) ← on slot promotion: end old, create + go_live new

One-time setup:
  1.  python youtube_broadcast.py --auth   (run once on your PC)
  2.  Paste token.json contents → YOUTUBE_TOKEN_JSON GitHub secret
  Done forever.
"""
...
def _hashtags(d: dict, is_shorts: bool = False) -> str:
    """Build hashtag string for description body."""
    tags = set()
...
def build_seo_title(d: dict, is_shorts: bool = False) -> str:
    """Clean SEO title. No score. Set once at broadcast creation."""
    t1     = d.get("team1", "")
    t2     = d.get("team2", "")
...
def build_seo_description(d: dict, is_shorts: bool = False) -> str:
    """Clean description. No score. No automation text."""
    t1    = d.get("team1", "")
    t2    = d.get("team2", "")
    desc  = d.get("match_desc", "")
    fmt   = (d.get("match_format") or "").upper()
...
class _TokenManager:
    ...
    def load(self) -> bool:
        ...
        if not raw:
            ...
            if local.exists():
                raw = local.read_text()
        if not raw:
            log.warning("YOUTUBE_TOKEN_JSON not set — broadcast unavailable")
            return False
        try:
    ...
    def token(self) -> str:
        ...
        if not all([rt, ci, cs]):
            raise RuntimeError("Token JSON missing required fields")
...
# ── Stream session — correct architecture ─────────────────────────────────────

class StreamSession:
    """
    Manages one YouTube streaming session for the duration of a GitHub Actions job.

    Architecture:
      - ONE liveStream object created at init → gives permanent RTMP URL
      - Multiple liveBroadcasts created over time (one per match)
      - Each broadcast is bound to the same liveStream
      - FFmpeg always streams to the same RTMP URL — never restarts

    Usage:
      session = StreamSession(is_shorts=False)
      if session.available:
          rtmp_url = session.rtmp_url          # give this to FFmpeg
          session.create_broadcast(match_data)  # call when match starts
          session.go_live()                    # call 10s after FFmpeg starts
          # ... match plays ...
          session.end_broadcast()              # call when match ends
          # slot promotion:
          session.promote(new_match_data)      # creates new broadcast, goes live
    """
    ...
    def _req(self, method: str, url: str,
        ...
        try:
            ...
            err = e.read().decode()
            raise RuntimeError(f"YT API {method} {e.code}: {err}")

    def _create_stream(self):
        """Create ONE liveStream. Called once at init. Gives permanent RTMP URL."""
        label = "Shorts" if self._is_shorts else "Main"
        try:
            stream = self._req("POST", f"{API}/liveStreams", body={
                "snippet": {"title": f"Xstar Sports — {label}"},
                "cdn": {
                    "frameRate":     "30fps" if not self._is_shorts else "variable",
                    "ingestionType": "rtmp",
                    "resolution":    "1080p",
                },
            ...
            log.info(f"liveStream created: {self._stream_id} ({label})")
        except Exception as e:
    ...
    def create_broadcast(self, match_data: dict) -> bool:
        """
        Create a liveBroadcast and bind it to the existing liveStream.
        Call this when a new match starts. Title/description set here, never updated.
        """
        ...
        try:
            # Create broadcast — FIX: categoryId is a snippet field, NOT status
            broadcast = self._req("POST", f"{API}/liveBroadcasts", body={
                "snippet": {
                    "title":              title,
                    "description":        description[:5000],
                    "scheduledStartTime": start_time,
                    "categoryId":         "17",          # Sports — correct: snippet field
                },
                "status": {
                    "privacyStatus":           "public",
                    "selfDeclaredMadeForKids": False,
                    # FIX: no 'tags' here — liveBroadcasts API does not support tags
                },
                "contentDetails": {
                    "enableAutoStart":   True,
                    "enableAutoStop":    False,  # False = broadcast survives FFmpeg crash/restart
            ...
            # Bind broadcast to the EXISTING liveStream
            ...
            log.info(f"Broadcast {self._broadcast_id} bound to stream {self._stream_id}")
            return True
    ...
    def go_live(self, max_wait: int = 120) -> bool:
        """Wait for stream to be active, then transition broadcast to LIVE."""
        ...
        for _ in range(max_wait // 5):
            ...
            log.warning("Stream not active after wait — attempting go_live anyway")
        ...
        try:
            ...
            err_str = str(e)
            # redundantTransition = broadcast already live (enableAutoStart did it)
            # This is success, not failure
            if "redundantTransition" in err_str or "redundant" in err_str.lower():
                log.info(f"Broadcast {self._broadcast_id} already LIVE (auto-started) ✅")
                return True
    ...
    def end_broadcast(self) -> bool:
        """End the current broadcast. liveStream stays alive for next match."""
    ...
    def promote(self, match_data: dict) -> bool:
        """
        Slot promotion: end current broadcast, create new one for next match.
        liveStream (and RTMP URL) stays the same — FFmpeg keeps streaming.
        """
        log.info("Slot promotion — ending current broadcast, creating new one")
        self.end_broadcast()
        ok = self.create_broadcast(match_data)
        if ok:
            # Small delay then go live
            time.sleep(3)
...
def run_auth():
    ...
    print(f"\n✅ Saved: {token_path}")
    print("\nCopy this into GitHub secret  YOUTUBE_TOKEN_JSON :")
    print("-" * 60)
