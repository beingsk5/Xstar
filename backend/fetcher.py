        ...
        while self.running:
            ...
            if now - self._last_medium >= POLL_MEDIUM_SEC:
                try:
                    ...
                    # Match complete — with Super Over protection
                    state   = (stream.get("state")  or "").lower()
                    status  = (stream.get("status") or "").lower()
                    inn_num = int(stream.get("innings_num") or 1)
                    # Super Over uses innings 3+: never close during super over
                    is_super_over = (
                        inn_num >= 3
                        or "super over" in status
                        or "superover" in status
                    )
                    # Only mark complete on true match-end states, not interruptions
                    # "No result", "Rain - No result" etc should NOT close stream
                    # Only close on "complete" state (not "no result") + explicit complete
                    is_truly_complete = (
                        ("complete" in state and "no result" not in state)
                        or state == "result"
                        or ("won" in status and "result" not in state)
                    )
                    if is_truly_complete and not is_super_over:
                        merged["stream_complete"] = True
                        WRITER.write(self.out_file, merged)
                        log.info(f"{self.L()} Match complete — stream will close in 10min")
                    elif is_super_over and is_truly_complete:
                        log.info(f"{self.L()} Super Over in progress — NOT closing stream")
