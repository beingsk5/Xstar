...
def _build_toss(toss: dict) -> str:
    """Build readable toss string: 'India won the toss and elected to bat'"""
    if not toss:
        return ""
    winner = toss.get("tossWinnerName", toss.get("tossWinner", ""))
    decision = (toss.get("decision", "") or "").lower()
    if not winner:
        return ""
    if decision in ("bat", "batting"):
        action = "elected to bat"
    elif decision in ("field", "bowl", "bowling", "fielding"):
        action = "elected to field"
    else:
        action = f"elected to {decision}" if decision else ""
    return f"{winner} won the toss and {action}" if action else f"{winner} won the toss"
...
def fetch_summary(match_id: str) -> dict:
    ...
    # Win probability — normalise to 0–1, mapped to "team1"/"team2" keys
    # Cricbuzz returns arbitrary keys (homeTeam/awayTeam or teamIds)
    # Map by position: first key → team1, second key → team2
    raw_wp = data.get("winProbability", {})
    win_prob = {}
    if raw_wp and isinstance(raw_wp, dict):
        vals = []
        for k, v in raw_wp.items():
            try:
                fv = float(v)
                # Normalise: Cricbuzz returns 0-100 range (percent)
                # Sometimes returns 0-1 range (fraction)
                # Sometimes returns raw integers > 100 (divide by 100 again)
                if fv > 100:
                    fv = fv / 100.0   # basis-point or scaled
                if fv > 1:
                    fv = fv / 100.0   # percentage → fraction
                vals.append(round(fv, 3))
            except Exception:
                pass
        if len(vals) >= 2:
            win_prob = {"team1": vals[0], "team2": vals[1]}
        elif len(vals) == 1:
            win_prob = {"team1": vals[0], "team2": round(1 - vals[0], 3)}
    ...
    return {
        ...
        "toss":          _build_toss(hdr.get("toss") or {}),
...
def fetch_live(match_id: str) -> dict:
    ...
    non_striker = (
        ms.get("batsmanNonStriker")
        or ms.get("batNonStrikerDetails")
        or ms.get("nonStrikeBatsman")
        or ms.get("nonStriker")
        or {}
    )
...
def fetch_commentary_page(match_id: str, page: int) -> list[dict]:
    ...
    Falls back to over-refresh commentary if full endpoint returns empty.
    """
    data = _get(f"{BASE}/mcenter/{match_id}/full-commentary/{page}")
    if not data:
        # Fallback: get from over-refresh endpoint
        data = _get(f"{BASE}/mcenter/over-refresh/{match_id}") or {}
