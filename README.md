...
| `YOUTUBE_MAIN_KEY` | Manual stream key fallback (no shorts) | Optional |
| `DISCORD_WEBHOOK` | Discord webhook URL for crash alerts | Optional |

> **YOUTUBE_MAIN_KEY** — only needed as a fallback if you cannot use `YOUTUBE_TOKEN_JSON`.
> With `YOUTUBE_TOKEN_JSON` set, both Main and Shorts streams are created automatically on your channel.
...
|--------|---------|----------------|
| `YOUTUBE_TOKEN_JSON` | OAuth2 token — handles both Main and Shorts | Run `python backend/youtube_broadcast.py --auth` |
| `YOUTUBE_MAIN_KEY` | Fallback if no token (main stream only, no shorts) | YouTube Studio → Go Live → Stream Key |
...
| Only main stream | Set YOUTUBE_TOKEN_JSON — both streams are automatic |
